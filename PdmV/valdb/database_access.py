...
def newRelease(cat, sub_cat, rel_name, dict_json, Session, *args):
    ...
    try:
        ...
        for status_kind in list:
            ...
            comments = dict[status_kind][COMMENTS]
            if comments: #if not None type
                comments = comments.encode('ascii', 'replace')
            links = dict[status_kind][LINKS]
            if links:
                links = links.encode('ascii', 'replace')
            status = Status_Table(id, dict[status_kind][VALIDATION_STATUS], comments, links, user_name, dict[status_kind][MESSAGE_ID], dict[status_kind][EMAIL_SUBJECT], dict[status_kind][RELMON_URL])
            session.add(status)
            ...
            for i in session.query(Releases_Table).filter(Releases_Table.category == cat).\
                                                    ...
                                                    filter(Releases_LV_Table.status_kind == status_kind):
                id_lv = i.id
            status_lv = Status_LV_Table(id, dict[status_kind][VALIDATION_STATUS], comments, links, user_name, dict[status_kind][MESSAGE_ID], dict[status_kind][EMAIL_SUBJECT],dict[status_kind][RELMON_URL])
